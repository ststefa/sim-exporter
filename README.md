...
The values will change over time based on the configured parameters. They can be either constant (`min = max`) or a range (`min < max`). Ranged values change over time in the simulator. Constant values (obviously) don't.
...
In case you want to finetune the simulation you can of course manually change the converted file and specify values, intervals and functions that make most sense to you.
...
Starts the interval in the middle (`(min+max)/2`) and does a full sine wave with the amplitude of `max-min`. Ends the interval in the middle again.
