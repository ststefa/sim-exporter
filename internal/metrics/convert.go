...
func convertScrapeToConfig(scrapeLines *[]string, maxdeviation int) (*Configuration, error) {
	...
	for lineno, line := range *scrapeLines {
		...
		if strings.HasPrefix(line, "# HELP") {
			...
			if !expectHelp {
				return nil, &errors.SimulationError{Err: fmt.Sprintf("Line %v: Unexpected 'HELP' (no 'TYPE' since last 'HELP')", lineno)}
			}
			expectHelp = false
			expectType = true
			...
			if !expectType {
				return nil, &errors.SimulationError{Err: fmt.Sprintf("Line %v: Unexpected 'TYPE' (not preceded by 'HELP')", lineno)}
			}
			expectType = false
			expectMetric = true
			fields := strings.Split(line, " ")
			...
			if fields[2] != metricName {
				return nil, &errors.SimulationError{Err: fmt.Sprintf("Line %v: Out-of-order line %q (expecting 'TYPE %v ...')", lineno, line, metricName)}
			}
			if !skipMetric {
				mType := fields[3]
			...
			if !expectMetric {
				return nil, &errors.SimulationError{Err: fmt.Sprintf("Line %v: Unexpected %q (not preceded by 'HELP')", lineno, line)}
			}
			expectType = false
			expectHelp = true
			...
			if !skipMetric {
				...
				if len(matchMap) >= 1 {
					...
					if err != nil {
						return nil, &errors.SimulationError{Err: fmt.Sprintf("Line %v: Cannot generate random number?!?", lineno)}
					}
...
func ConvertScrapefileToYaml(filename string, maxdeviation int) ([]byte, error) {
	...
	config, err := convertScrapeToConfig(scrapeLines, maxdeviation)
	if err != nil {
		return nil, err
	}
